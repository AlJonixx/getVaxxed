...
from .forms import CreateDvdForm, CreateUserForm, LoginForm, UpdateForm
    ...
    def post(self, request):
        ...
        if request.method == 'POST':  
            ...
            if 'btnUpdateUser' in request.POST:
                ...
                update_user = User.objects.filter(id = userid).update(email = postEmail, username = postUsername, first_name = postFirstname, last_name = postLastname)
                messages.success(request, 'User ' + postUsername + ' has been updated successfully.')

            if 'btnApprovePost' in request.POST:
                status = 'APPROVED'
                postid = request.POST.get("postID")
                approve_post = Post.objects.filter(postPK = postid).update(post_status = status)
                print(postid)
